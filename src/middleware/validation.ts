...
export const validateBookingRequest = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (appointmentType && !validAppointmentTypes.includes(appointmentType as AppointmentType)) {
...
export const validatePaymentStatusUpdate = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (currency && !/^[A-Z]{3}$/.test(currency)) {
    errors.push('currency must be a valid 3-letter ISO 4217 currency code (e.g., USD, EUR)');
  }
 ...
 */
export const validateNanoId = (req: Request, res: Response, next: NextFunction): void => {
